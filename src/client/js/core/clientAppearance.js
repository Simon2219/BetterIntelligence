...
export function createAppearanceController({
    fetchResolvedAppearance: fetchResolvedAppearanceImpl = fetchResolvedAppearance,
    applyThemeVariables: applyThemeVariablesImpl = applyThemeVariables,
    api
} = {}) {
    async function applyAppearance() {
        try {
            const data = await fetchResolvedAppearanceImpl(api);
            const theme = document.documentElement.getAttribute('data-theme') || 'dark';
            applyThemeVariablesImpl(theme, data);
        } catch {}
    }
